import DJSError from "./DJSError.ts";
...
const Messages = {
  ...
  SHARDING_SHARD_NOT_FOUND: (id: string | number) => `Shard ${id} could not be found.`,
  SHARDING_ALREADY_SPAWNED: (count: number) => `Already spawned ${count} shards.`,
  SHARDING_PROCESS_EXISTS: (id: string | number) => `Shard ${id} already has an active process.`,
  SHARDING_WORKER_EXISTS: (id: string | number) => `Shard ${id} already has an active worker.`,
  SHARDING_READY_TIMEOUT: (id: string | number) => `Shard ${id}'s Client took too long to become ready.`,
  SHARDING_READY_DISCONNECTED: (id: string | number) => `Shard ${id}'s Client disconnected before becoming ready.`,
  SHARDING_READY_DIED: (id: string | number) => `Shard ${id}'s process exited before its Client became ready.`,
  SHARDING_NO_CHILD_EXISTS: (id: string | number) => `Shard ${id} has no active process or worker.`,
  SHARDING_SHARD_MISCALCULATION: (shard: number, guild: string, count: number) =>
  ...
  FILE_NOT_FOUND: (file: unknown) => `File could not be found: ${file}`,
  ...
  VOICE_STATE_INVALID_TYPE: (name: number) => `${name} must be a boolean.`,
...
for (const [name, message] of Object.entries(Messages)) DJSError.register(name, message);
